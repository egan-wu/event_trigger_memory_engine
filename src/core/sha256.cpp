...
std::string sha256_hex(const std::string& data) {
    // FIPS 180-4 initial hash values: the first 32 bits of the fractional
    // parts of the square roots of the first 8 primes.
    uint32_t h[8] = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
                     0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};

    // Padding: a single 1-bit (0x80 byte, since input is byte-aligned), then
    // zero bytes until length % 64 == 56, then the original bit length as a
    // big-endian 64-bit integer -- brings the total to a multiple of 64.
    std::string msg = data;
    ...
    for (uint32_t hv : h) os << std::setw(8) << hv;
    return os.str();
}
